...
Removing the layer then changes conserving dynamics through feedback. We distinguish output coordinates, projection during training,
and projection during inference with an eight-cell intervention design. Projected training
identifies the deployed predictor only up to an unconstrained raw invariant component. In a
prospectively frozen, 30-seed PDEBench Advection study, the three-way interaction is
$\CubeJ$ [$\CubeJLow$, $\CubeJHigh$], material in \CubeMaterialCells/12 mandatory
resolution--horizon cells. Opposite-signed training and inference credits obscure this dependence
in the ordinary bundled comparison; removal damage emerges through autoregressive feedback.
An independently frozen, 30-seed shallow-water study reproduces material path dependence in
\SWEMaterialCells/8 cells. A direct input intervention also establishes that changes along the
invariant direction can affect conserving dynamics in the tested FNOs. The registered U-Net
transfer has an unresolved primary interaction and only \UNetMaterialCells/12 material cells;
post-hoc replay places its eight native-grid primary cells below persistence. A separate post-hoc
U-Net deployment control, computed on a fixed spatial grid, is predictive and exhibits path
dependence, but changes the deployment strategy and does not replace that registered transfer. These results show that, in the tested FNOs,
the conservation layer participates in the learned autoregressive transition. Deployment changes
that bypass it therefore require validation of the resulting rollout dynamics.
\end{abstract}
...
\paragraph{Projection participates in the learned transition.}
The two frozen FNO studies establish material dependence on the inference projector. In Advection,
same-checkpoint projection leaves conserving error unchanged at horizon one, while removing it
produces material interactions at every registered horizon-four cell. The direct input intervention
confirms that the exposed invariant component changes the next conserving prediction. Together,
the intervention cube and the input test locate a learned feedback pathway through the projector.

\paragraph{Bundled comparisons conceal opposing effects.}
The Advection training and inference credits are large and opposite-signed, leaving a small
bundled interaction after cancellation. The eight-cell design recovers these separate effects
by adding and removing projection on both free- and hard-trained checkpoints with fixed weights.
The projection-gauge result explains the corresponding identification limit: the projected loss
leaves the raw invariant component unconstrained. Raw outputs require their own predictive
validation when deployment bypasses the projector.

\paragraph{Evaluate conserving dynamics as well as exact enforcement.}
Conserving error $\lVert Qe\rVert$ measures predictive changes outside the mechanically corrected
channel, while invariant error $\lVert Pe\rVert$ measures enforcement. Both are needed to assess
rollout changes; positivity requires its separate compatibility condition
(Proposition~\ref{prop:positivity}). The registered U-Net result and its post-hoc resolution adapter
also make deployment strategy part of the empirical scope. In the two frozen FNO studies,
conservation enforcement is a component of the trained dynamics. Evaluating it jointly during
training and rollout exposes dependence that the ordinary bundled comparison misses.
...
confirmation. The resolution-adapter view additionally contains 192 native/adapted skill cells and paired trajectory errors for CPU replay. Full checkpoints and trajectory arrays are retained in the recovery archive.
...
by $|\alpha|$, using amplitudes $\alpha/\sigma\in\{-0.5,-0.1,0.1,0.5\}$.
