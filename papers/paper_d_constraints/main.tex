...
\begin{abstract}
Exact conservation layers guarantee admissible outputs, but their removal can change the dynamics
of an autoregressive predictor. We distinguish output coordinates, projection during training,
...
An independently frozen, 30-seed shallow-water study has material path dependence in
\SWEMaterialCells/8 cells, although residual models remain near persistence in a checkpoint replay. A direct input intervention also establishes that changes along the
...
a conservation layer can become part of the learned autoregressive transition. A second
Advection recipe retains the interaction, while equal-RMS direction controls limit any
claim that the feedback response is specific to the invariant direction. Exact enforcement
alone does not justify treating that layer as a detachable post-processor.
\end{abstract}
...
block-averages history onto 256 cells, evaluates the U-Net there, and periodically interpolates
the output back to the target cell centers before feedback. No weights change. On the same 256
...
show median absolute-coordinate removal harm 74.71 (IQR 42.71--128.71), positive in 28/30
seeds, showing broad seed-level harm within this fixed test cohort
...
\paragraph{Physical-history response.}
A post-hoc Advection control shifts all ten input frames by a constant or adds an
analytically advecting sinusoid. Under whole-history constant shifts, OOD-512 conserving
residuals are 0.0083--0.0106 across the four arms, with positive hard-minus-free contrasts
in both coordinates. Exact output conservation does not establish better equivariance
to this physical transformation. Appendix~\ref{app:revision-history} gives amplitudes,
analytic references and uncertainty.

\paragraph{Local gradient boundary.}
The frozen one-Adam-step intervention has an absolute-minus-residual effect of
$\CubeOneStepInteraction$ [$\CubeOneStepInteractionLow$, $\CubeOneStepInteractionHigh$].
Its association with the final inference-matched training interaction is unresolved:
$\rho=\CubeMechanismRho$ [$\CubeMechanismRhoLow$, $\CubeMechanismRhoHigh$].
The direct gauge intervention establishes a feedback channel, but this local gradient
diagnostic does not explain the final seed-level variation.
...
Evaluation should add and remove projection on each locked checkpoint, separating
output coordinates, training and deployment. An asymmetric comparison that only adds
projection to free models cannot detect learned removal dependence. Raw outputs of
projected models are identified only modulo the invariant direction; bypassing the
projector can change the autoregressive transition even when one-step conserving
predictions agree. Report invariant and conserving errors separately, alongside
forecast skill and trajectory quality. The observed feedback channel need not be
direction-specific, and large interaction magnitude need not imply useful prediction.

\section{Limitations}

The experiments cover two PDEBench scenarios, uniform mass and resolution shifts.
They do not establish generality over coefficients, nonlinear invariants or initial
conditions. Both independent path-dependence studies use FNOs. Shallow-water residual
cells remain near persistence; the depth-only surrogate does not establish full-system
fidelity. Native-grid U-Net is below persistence, while its predictive post-hoc adapter
changes deployment and is not capacity matched. Gauge non-identification permits,
but does not force, unstable raw predictions. Credits and materiality thresholds
condition on the fixed cube. Seed-only intervals condition on trajectories; the
crossed bootstrap additionally resamples the observed trajectories, with pointwise
post-hoc intervals. Equal-RMS controls test local response specificity, not rollout
robustness. Advection's constant-shift and advecting-sine controls do not apply to
shallow-water depth.
...
observations. The authors take responsibility for the final content,
...
the original network on that grid and interpolates its output periodically to the target cell
centers. The interpolated prediction enters the next history at the target resolution. For an
