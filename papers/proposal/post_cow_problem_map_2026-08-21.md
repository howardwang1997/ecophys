...
```mermaid
flowchart TD
    ROOT[Impactful deployed mechanism] --> OLD[Closed EcoMD / invariant-calibration routes]
    ROOT --> FIELD[Real field mechanisms]
    FIELD --> POWER[Electricity forecast / RTC+B]
    FIELD --> DEFI[Automated economic control]
    FIELD --> AGENTS[Agent marketplaces]

    POWER --> P0[Forecast performativity RED: not identified]
    POWER --> P1[RTC+B parked: 2 bundled clusters]
    DEFI --> AAVE[Aave guardrail RED: no local threshold support]
    DEFI --> LIQ[Liquity RED: source mismatch]
    DEFI --> MORPHO[Morpho RED: identity / transport gates]
    AGENTS --> COW[CoW RED: 132/200 units; sparse coupling]

    ROOT --> NEXT[Fresh problem selection]
    NEXT --> CAL[D-1 outcome-blind rate/schema calibration]
    CAL --> ID{Repeated intervention or built-in counterfactual?}
    ID -- no --> STOP[Do not consume a holdout]
    ID -- yes --> FREEZE[Freeze claim, sample size and kill rules]
    FREEZE --> D0[Formal support]
    D0 --> OUT[Only then open outcomes]
```
