...
```mermaid
flowchart TD
    E[EcoMD / mechanism-exact hybrid idea] --> C[Compound cap route]
    E --> A[Aave scalar LT route]
    C --> C0[D0/D0b mechanics: 4 cap increases]
    C0 --> C1[D1a exact T-1 activation]
    C1 --> CF[FAIL: 0 exactly saturated caps]
    A --> A0[A0 source/effect identity: PASS]
    A0 --> A1[A1a Ethereum event directory]
    A1 --> AF[FAIL: 0 strict unbundled LT-only candidates]
    A1 --> PD[Protocol diagnostic: initial proxy emits no Upgraded]
    PD --> HC[Source-corrected history conforms]
    HC --> AF
    CF --> S[Shared lesson: executable interventions are programs, not scalar labels]
    AF --> S
    ...
    G1 -->|pass| M[Multiscale adaptation model + long-horizon calibration]
    G1 -->|fail| R[Retire/reframe; no participant model]
    M --> NCS[NCS: computational method + scientific insight]
    M --> NMI[NMI stretch: strong OOD ML novelty + broader benchmark]
```
