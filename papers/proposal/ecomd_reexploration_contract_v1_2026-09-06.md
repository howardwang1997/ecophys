...
> **Pre-freeze amendments 2026-09-06 (PI rulings on the D-1 session-1 decision items, "用需要算力多的那个，批准，确认，确认"):**
> (A) C2: K = **16** (K-variance preflight rule output `none_in_set` → 16; PI chose the compute-heavier option, ~+15% Stage-1 eval V100-h; K now immutable after D0). (B) C3: kernel sentence tightened — engine kernels are {fifo, random_unit_within_price}; `pro_rata` is S3 exact-arithmetic only, never an engine kernel. Both recorded as C16 ledger items (11)–(12). Derivation sections in Parts 1–2 retain their original K = 8 wording as historical reasoning superseded by the frozen clauses below.
...
**C2 — Randomness contract.** (a) Primary-truth DGP policies are frozen in the M0/M1-lumpable class (actions depend only on anonymous book state, own inventory, cash, induced values — never on allocation identity), so executed request streams are kernel-invariant and request-level CRN is exact. (b) **K = 16** inference-kernel draws per seed per cell [amended 2026-09-06 from the candidate K = 8: the frozen conservative K-preflight rule returned `none_in_set` → K = 16 (`experiments/reexploration/k_preflight_20260906/`, results sha256 f61e410c…), and the PI selected the compute-heavier option], derived `spawn(seed, "kernel", k)`, k = 1…16, replayed identically across cells within a seed; per-seed cell value = K-draw mean. (c) Deterministic-kernel cells execute K = 16 identical evaluations; byte-identity is gate G8. (d) Neural decode is deterministic at eval; no sampling heads. (e) The pre-D0 K adjustment window is now CLOSED: K = 16 frozen by PI ruling 2026-09-06; after D0, K is immutable. `alloc_proprata_dev_mean` remains draw-dominated at every authorized K (0.196 at K = 16) — this statistic carries an explicit draw-noise caveat in all reporting.

**C3 — Cube structure and estimands.** Trained arms: (c, t), c ∈ {absolute next state, increment}, t ∈ {raw, through-M}; each evaluated at inference e ∈ {raw, through-M} → 8 cells Y_{c,t,e}. D_{te} = Ȳ_absolute,te − Ȳ_increment,te; T_0 = D_00 − D_10; T_1 = D_01 − D_11; E_0 = D_00 − D_01; E_1 = D_10 − D_11; **J = D_00 − D_10 − D_01 + D_11**; φ_train = (T_0 + T_1)/2, φ_infer = (E_0 + E_1)/2, φ_train + φ_infer = D_00 − D_11. Sign convention: positive J means the absolute-minus-increment coordinate contrast grows with joint through-M enforcement. M = the exact combinatorial clearing layer (volume and cash conservation, unit integrality, tick lattice, priority kernel ∈ **{fifo, random_unit_within_price}** — the two and only two kernels implemented by the lab-asset-v3 engine (`scripts/lab_asset/schema.py:22-24`); **`pro_rata` is NOT an engine kernel and never executes in any cell** — it exists only as S3 exact-arithmetic in theorem/audit calculations [tightened 2026-09-06 per contracts-doc finding + PI confirmation]); through-M training uses the preregistered surrogate family frozen at D0. **No confirmatory test compares through-M vs raw for superiority; the estimand is path-dependence attribution.**
...
- **G8 Deterministic-kernel draw identity**: fifo cells' K = 16 draw-records byte-identical (pro_rata is not an engine kernel and has no cells; see C3).
- **G9 Determinism replay**: frozen 5% seeded sample of records re-executed byte-identically.
- **G10 Field validity**: finite metrics; `pre/post_best_bid/ask` present; `allocation_rule` matches the cell/axis spec; `n_draws == 16` (C2 amendment).
