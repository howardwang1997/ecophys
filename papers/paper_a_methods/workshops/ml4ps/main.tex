...
% !! STALE 2026-06-23: main.md was revised (stationary -> "no crash heavy-up" softening, alpha=0.5
%   censored-floor, tau_OFI labeling, event names, four-classes, C-a irreversibility pending).
%   Regenerate the body from main.md (source of truth) before building/submitting.
...
\citep{cont2001,gabaix2003,plerou1999}, and is usually treated as a \emph{stationary} property of the
return distribution to be explained by an equilibrium mechanism \citep{gabaix2003}. A dissenting
tradition instead reads it as \emph{dynamics-generated}: a finite-variance process under a fluctuating
volatility clock \citep{clark1973}, or apparent power-law behaviour produced by multi-timescale
stochastic volatility \citep{lebaron2001}. Separating these views observationally is hard, because one
cannot intervene on a real market and watch its tail relax. Mechanistic, particle-based simulators can:
in a driven many-body system whose macroscopic observable is the price, ``steady state'' and
``relaxation'' are precise, and we can ask whether the heavy tail is a stationary equilibrium property
or a \emph{non-equilibrium transient}---and whether the standard way of measuring it tells the two apart.
...
identical to the unshocked control. Under the coherent shock, order flow shows a clean, quantitatively
characterized signature (Fig.~\ref{fig:mech}b): the lag-1 autocorrelation of order-flow imbalance (OFI)
jumps from $0.02$ to $\sim\!\mathbf{1.0}$ at the shock---flow becomes transiently coherent and
persistent---and relaxes back. It has a \textbf{clean monotonic sigmoid dose-response} (onset
$\sim\!0.1$--$0.2\sigma\to\sim\!1.0$ by $\sim\!1\sigma$), \textbf{generalizes across 4 of 5 assets} (the
low-vol FX pair is sub-threshold, its onset tracking intrinsic volatility), and relaxes on a
\textbf{sharp timescale $\tau_{\mathrm{OFI}}\!\approx\!22$ steps---$\sim$10$\times$ faster than the
return-tail $\tau_{\mathrm{ED}}\!\approx\!236$}, giving the system \emph{two} non-equilibrium timescales.
\textbf{On the thermodynamic reading (honest):} we computed a sign-level entropy-production proxy on
$(\Delta p,\text{OFI})$---the KL divergence between forward and time-reversed pair-transition
statistics---and it was \textbf{flat}. So the signature is order-flow \emph{persistence/coherence}, not
sign-level \emph{irreversibility} (a persistent, AR-like process can be time-reversible, and entropy
production need not accompany the memory burst). A finer-grained entropy-production estimate---connecting
to fluctuation-theorem studies of market cascades \citep{maskawa2025} and non-equilibrium stochastic
thermodynamics \citep{seifert2012}---is a concrete next step we do not claim to have established here.
...
inverse-cubic universality \citep{gabaix2003,toth2011}. The simulator's transient heavy tail is
