...
state---which we use as the non-equilibrium probe.
...
right). The heavy tail is therefore a property of the \emph{driven}, not the steady, state.
...
and connects to fluctuation-theorem descriptions of market cascades \citep{maskawa2025} and to
non-equilibrium stochastic thermodynamics more broadly \citep{seifert2012}.
...
source, it can produce heavy tails only transiently. This localizes the missing ingredient---a
stationary heavy-tail mechanism (e.g.\ a heterogeneous, heavy order-flow source)---for this class of
simulators.
