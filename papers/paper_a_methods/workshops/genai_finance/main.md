...
0.02 to ~1.0 at the shock with a clean monotonic dose-response, generalizing across assets and relaxing
sharply — whereas exogenous price gaps are inert. Finally, we bound fidelity honestly: the model's heavy
...
demand. Differentiable agent-based models are not new [Bouchaud & Cont 1998; Dyer et al. 2024; Chopra
...
- **Honest fidelity bounds** (§5): generated tails are transient while real crash tails are
...
agent-based simulator [Byrd et al. 2020, ABIDES] does not have: gradient-based calibration, and a clean
*shock interface* for controllable interventions (§4). The rest of the paper concerns how to evaluate
...
to the empirical cube law. But it is a startup artifact. Discarding the first ~50 steps moves the
estimate sharply toward the light steady state: on EcoMD, the baseline Hill index rises from 3.87 to
6.55, and a concave-impact variant tuned to "match" the cube law rises from 5.05 to 9.26 (Table 1;
Figure 1). The steady state is *light*-tailed (*α ≈ 4.7* for excess demand); the apparent fat-tail
match was burn-in inflation. **Crucially the artifact is *rewarded*, not caught:** the
...
positions in a common direction. This drives a clean, *relaxing* stress scenario: the heavy transient
tail re-appears (*α* for excess demand drops from 4.7 to ~0.5 and recovers within ~10³ steps), with a
graded, sigmoid dose-response. It also leaves a distinctive, **controllable order-flow signature**, which
we characterize quantitatively (Figure 2, left):
...
- *Dose-response.* The OFI-memory burst is a **clean monotonic sigmoid** in shock magnitude (onset
...
- *Generality.* The signature reproduces across **four of five assets** (equities, NASDAQ, gold,
  ...
  volatility.
- *Timescale.* The OFI-memory burst is a **sharp impulse** (relaxation τ ≈ 22 steps), ~10× faster than
...
*finite-time relaxation*, its *graded dose-response*, and the sharp contrast with the inert price gap.
...
compared it to a null distribution built from a long calm window (Figure 2, right). If crashes drove a
...
within the false-positive rate for five tests. Real return tails are *stationary* — approximately
cube-law in calm and crash alike, consistent with the inverse-cubic universality [Gabaix et al. 2003;
Plerou et al. 1999; Tóth et al.]. This uses the five episodes available with free intraday data
...
tail. For scenario generation this is a sharp caveat: the simulator reproduces fat tails only
...
drives realistic stress in this generator, via a sharp, dose-responsive order-flow-coherence burst.
...
et al.; Maskawa 2025. *(full entries in `references.bib`)*
