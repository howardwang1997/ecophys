...
%   censored-floor, tau_OFI labeling, event names, four-classes, goodness-of-fit sentence).
...
lag-1 autocorrelation of order-flow imbalance (OFI) jumps from $0.02$ to $\sim\!1.0$ at the shock---order
flow becomes transiently coherent and persistent---and relaxes back, with the imbalance magnitude and
its extreme-coordination saturation bursting in step (Fig.~\ref{fig:control}, left). This OFI-memory
burst is a \textbf{clean monotonic sigmoid} in shock magnitude (onset $\sim\!0.1$--$0.2\sigma$,
saturating $\sim\!1.0$ by $\sim\!1\sigma$)---a sharper control knob than the tail---reproduces across
\textbf{4 of 5 assets} (the low-vol FX pair is sub-threshold, tracking its volatility), and is a
\textbf{sharp impulse} ($\tau_{\text{OFI}}\!\approx\!22$ steps, $\sim$10$\times$ faster than the tail
relaxation $\tau\!\approx\!236$). A sign-level entropy-production proxy on $(\Delta p,\text{OFI})$ was
\emph{flat}, so the signature is order-flow coherence/persistence, not sign-level irreversibility.
...
also a concrete, falsifiable prediction to test against real crash order-flow data---a natural place to
look, since the non-trivial dynamical structure of markets is known to reside in order flow, which
carries long memory while returns stay near-efficient \citep{lillo2004,cont2014,toth2011}, complementing
...
crash alike, consistent with the inverse-cubic universality \citep{gabaix2003,plerou1999,toth2011}.
