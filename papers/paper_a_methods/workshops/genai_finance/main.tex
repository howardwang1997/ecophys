...
augmentation. Their quality is judged almost universally by \emph{stylized-fact matching}: a rollout
is scored on whether its returns reproduce empirical regularities such as the heavy (inverse-cubic)
tail and volatility clustering \citep{cont2001,gabaix2003}. Two practical questions receive far less
attention than the models themselves. First, is this evaluation \emph{rigorous}---does a high stylized-fact
score actually certify a stationary match? Second, how \emph{controllable} are the generated
scenarios---which interventions produce which market regimes?

We study both through \textbf{EcoMD}, a differentiable particle simulator in which agents are
particles evolving under overdamped Langevin dynamics with learned pairwise potentials, and prices
form from aggregate excess demand. Differentiable agent-based models are not new
\citep{bouchaud1998,dyer2024,chopra2023}; our contributions are not the simulator's existence but three findings
about \emph{generating} and \emph{evaluating} market scenarios:
...
paths and the stylized facts EcoMD reproduces. As a \emph{generator}, then, EcoMD is a controllable,
trainable producer of market scenarios; the rest of the paper concerns how to evaluate and steer it.
...
($\alpha\!\approx\!4.7$ for excess demand); the apparent fat-tail match was burn-in inflation. The
effect has low cross-seed variance, so it masquerades as a robust law rather than an artifact.
...
\textbf{The fix.} Score rollouts \emph{after} a warm-up discard, and report a warm-up-sensitivity
curve ($\alpha$ vs.\ discard length) rather than a single number. This is a one-line change to any
rollout-scoring pipeline and it changes qualitative conclusions: under it, EcoMD's earlier ``cube-law
reproduction'' is withdrawn. We recommend it as default hygiene for evaluating any rollout-based
market generator.
...
The practical takeaway for scenario generation is that \emph{controllability is intervention-specific}:
the lever that produces realistic stress here is coordinated participant behavior, not an exogenous
price move. The coherent-liquidation order-flow signature is also a concrete, falsifiable prediction
to test against real crash order-flow data, complementing fluctuation-theorem analyses of market
cascades \citep{maskawa2025}.
...
its output can be trusted, and a measurement caveat that applies well beyond it.
