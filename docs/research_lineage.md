...
`papers/proposal/v14_experiment_data_compute_audit_2026-08-14.md`.
...
calibration reduces to Poisson/Stein, long-term Koopman and invariant-measure learning. The retained noisy-XOR
warning matches both observed one-step and stationary laws while differing in its three-time law, so long-horizon
calibration cannot repair an omitted state. The 135-node graph validates. Exp146 was not run; no data, workers or
GPU were used. The protocol and final outcome are `papers/proposal/plan_theory_exploration_nmi_ncs_v3.md`.
...
|---|---|---|---|
| Plan v3 market-physics program | Suspended/closed hypotheses | Data governance, preregistration discipline | Effective-temperature, Jarzynski, TUR and universal crash claims |
| EcoMD v1 model release | M1 failed; source preview only | Complete state, resume, packaging and stationarity gates | Validated simulator, real market physics or order-level fidelity |
...
1. Historical experiment directories and result artifacts are immutable.
