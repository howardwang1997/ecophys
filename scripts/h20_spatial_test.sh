#!/usr/bin/env bash
# Phase 7 — Spatial-checkpoint memory test, mirrors h20_hbm_profile.sh
# pattern. Runs each config sequentially on a single card to get clean
# peak-HBM measurements (not config-parallel).
#
# Output: peak HBM in each results dir's training_log.json (peak_hbm field).
# Aggregate: scripts/score_hbm_profile.py works for any results dir
# containing training_log.json with peak_hbm.
#
# Usage:
#   bash scripts/h20_spatial_test.sh
#   conda run -n ecophys python scripts/score_hbm_profile.py    # if needed
...
CONFIG_DIR="experiments/029_spatial_checkpoint"
QUEUE_LOG="$CONFIG_DIR/_run_$(date +%Y%m%d-%H%M%S).log"
: > "$QUEUE_LOG"

echo "═══ SPATIAL CHECKPOINT TEST — $(date) ═══" | tee -a "$QUEUE_LOG"
echo "  Card: $CARD, NPROC: $NPROC" | tee -a "$QUEUE_LOG"

cfgs=( "$CONFIG_DIR"/config_p7_*.yaml )
...
for d in "$CONFIG_DIR"/results_p7_*; do
