#!/usr/bin/env bash
# exp 125 — ORCHESTRATOR. Run this ON THE 2-CARD ORCHESTRATOR BOX. It SSHes to the
# 8-card box and the other 2-card box, syncs code+data, launches their job batches
# detached (nohup), runs the orchestrator's own local batch, then gathers + evals.
#
# The 4-card box is DOWN, so its G-A rigor load is migrated onto the 8-card.
#
# Node map is read from scripts/machines.local.json. Required keys (host/user/root/env/gpus):
#   - the box you run this on        = "localhost"  (2 cards; orchestrator + its own G-C slice)
#   - NODE_8CARD  (default: gpu8)    = the 8-card box (G-B atlas + G-A rigor + spx G-C/G-D)
#   - NODE_2CARD  (default: gpu2b)   = the other 2-card box (btc G-D + G-E)
# Override the keys: NODE_8CARD=h20_1 NODE_2CARD=h20_3 bash scripts/gpu_exp125_orchestrate.sh launch
#
# USAGE
#   DRY=1 bash scripts/gpu_exp125_orchestrate.sh launch    # echo every action, no SSH/GPU
#   bash scripts/gpu_exp125_orchestrate.sh launch          # sync + launch all 3 nodes
#   bash scripts/gpu_exp125_orchestrate.sh status          # tail logs + count npz on each node
#   bash scripts/gpu_exp125_orchestrate.sh gather          # rsync remote results back here
#   bash scripts/gpu_exp125_orchestrate.sh eval            # run eval on the gathered results
set -uo pipefail
...
say(){ echo "[orch $(date +%H:%M:%S)] $*" | tee -a "$LOG" >&2; }
...
launch(){
  ...
  say "4) run orchestrator-local batch (ndx+btc dose law) on these 2 cards"
  batch_local
...
status(){
  ...
  for key in "$NODE_8CARD" "$NODE_2CARD"; do
    ...
    say "── $key ($u@$h) ──"
    do_ "ssh -p $p $u@$h 'cd $r; ls ${EXP}/results_*/*/trajectory_*.npz 2>/dev/null | wc -l | sed \"s/^/  npz so far: /\"; tail -2 ${EXP}/_node_${key}_*.log 2>/dev/null'"
  done
...
gather(){
  say "═══ GATHER remote results → orchestrator ═══"
  local key h u r p
  for key in "$NODE_8CARD" "$NODE_2CARD"; do
    h="$(mach "$key" host)"; u="$(mach "$key" user)"; r="$(mach "$key" root)"; p="$(mach "$key" ssh_port)"; p="${p:-22}"
...
eval_all(){ do_ "bash scripts/gpu_exp125_atlas.sh eval"; }
