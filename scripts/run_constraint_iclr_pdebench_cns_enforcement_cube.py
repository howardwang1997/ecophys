"""Run the frozen zero-training PDEBench compressible-NS enforcement cube."""
...
from analyze_constraint_iclr_pdebench_cns_enforcement_cube import (
    DERIVED_SPEC,
    canonical_object_sha256,
    validate_cns_checkpoint_lock,
)
...
from run_constraint_iclr_pdebench_cns import (
    build_cns_model,
    evaluate_cns_cases,
    load_cns_trajectories,
    validate_cns_data_lock,
)
...
def _checkpoint_payload(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(f"CNS checkpoint payload is not a mapping: {path}")
    if payload.get("schema_version") != expected_schema:
        raise RuntimeError(f"CNS checkpoint schema mismatch: {path}")
    if payload.get("run_id") != expected_run_id:
        raise RuntimeError(f"CNS checkpoint run ID mismatch: {path}")
    if int(payload.get("completed_epochs", -1)) != expected_epochs:
        raise RuntimeError(f"CNS checkpoint epoch count mismatch: {path}")
    state = payload.get("model_state_dict")
    if not isinstance(state, dict) or not state:
        raise RuntimeError(f"CNS checkpoint has no model state: {path}")
    return payload


def cns_checkpoint_lock_payload(
    *,
    root: Path,
    ...
    for seed in [int(value) for value in config["seeds"]]:
        for mechanism in [
            ...
            path = resolve_path(root, raw_path)
            if not path.is_file():
                raise RuntimeError(f"CNS parent checkpoint is missing: {path}")
            observed_bytes = path.stat().st_size
            observed_sha256 = sha256_file(path)
            if observed_bytes != int(checkpoint.get("bytes", -1)):
                raise RuntimeError(f"CNS parent checkpoint byte count changed: {path}")
            if observed_sha256 != str(checkpoint.get("sha256")):
                raise RuntimeError(f"CNS parent checkpoint SHA-256 changed: {path}")
...
def create_or_validate_cns_checkpoint_lock(
    *,
    root: Path,
    core: dict[tuple[int, str], dict[str, Any]],
    config: dict[str, Any],
    ...
    payload = cns_checkpoint_lock_payload(
        root=root,
        core=core,
        config=config,
    ...
    if lock_path.exists():
        if lock_path.read_text(encoding="utf-8") != canonical:
            raise RuntimeError("existing CNS checkpoint lock is not byte-identical")
    else:
...
def validate_cns_checkpoint_files(
    *,
    root: Path,
    ...
    for entry in checkpoint_lock["checkpoints"]:
        ...
        if not path.is_file():
            raise RuntimeError(f"locked CNS checkpoint disappeared: {path}")
        if path.stat().st_size != int(entry["bytes"]):
            raise RuntimeError(f"locked CNS checkpoint byte count changed: {path}")
        if sha256_file(path) != str(entry["sha256"]):
            raise RuntimeError(f"locked CNS checkpoint SHA-256 changed: {path}")
        _checkpoint_payload(
            path,
...
def _existing_ids(path: Path) -> set[str]:
    ...
    for line_number, line in enumerate(
        ...
        try:
            ...
            raise RuntimeError(
                f"invalid CNS cube JSONL at line {line_number}"
            ) from error
        completed.add(str(record["run_id"]))
    return completed


def run_cns_cube(cfg: DictConfig) -> None:
    root = Path(__file__).resolve().parents[1]
    resolved = OmegaConf.to_container(cfg, resolve=True)
    if not isinstance(resolved, dict):
        raise TypeError("resolved CNS cube config must be a mapping")
    cube = resolved.get("cube")
    if not isinstance(cube, Mapping):
        raise TypeError("CNS cube config must be a mapping")
    if cube.get("stage") not in {
        "cns_enforcement_cube_confirmation",
        "cns_enforcement_cube_smoke",
    }:
        raise RuntimeError("unsupported CNS enforcement-cube stage")
    if [str(value) for value in cube["derived_mechanisms"]] != list(DERIVED_SPEC):
        raise RuntimeError("CNS derived mechanisms differ from the frozen cube")
    if int(cube["expected_derived_records"]) != 3 * len(resolved["seeds"]):
        raise RuntimeError("CNS derived record count differs from the seed list")
    if cube["stage"] == "cns_enforcement_cube_confirmation":
        if [int(value) for value in resolved["seeds"]] != list(range(5000, 5030)):
            raise RuntimeError("formal CNS cube seeds must remain 5000--5029")
        if int(cube["expected_core_records"]) != 150:
            raise RuntimeError("formal CNS cube requires exactly 150 core records")
        if int(cube["expected_trained_checkpoints"]) != 120:
            raise RuntimeError("formal CNS cube requires exactly 120 checkpoints")
        if int(cube["expected_derived_records"]) != 90:
            raise RuntimeError("formal CNS cube requires exactly 90 derived records")
        if len(resolved["evaluation"]["case_names"]) * len(
            resolved["evaluation"]["horizons"]
        ) != 12:
            raise RuntimeError("formal CNS cube requires all 12 evaluation cells")
    ...
    if cube["stage"] == "cns_enforcement_cube_confirmation":
        if not resolved.get("model_decision_path") or not resolved.get(
            ...
            raise RuntimeError("CNS cube requires the frozen formal model decision")
    ...
    if not core_records_path.is_file() or not core_analysis_path.is_file():
        raise RuntimeError("CNS cube activation requires complete core records and analysis")
    core_records = read_records(core_records_path)
    if len(core_records) != int(cube["expected_core_records"]):
        raise RuntimeError("CNS cube parent does not have exact core coverage")
    core = validate_cns_records(core_records, resolved)
    ...
    if completed_core_analysis.get("input_sha256") != sha256_file(core_records_path):
        raise RuntimeError("CNS core analysis is not bound to the parent JSONL")
    ...
    if data_lock_sha256 != str(resolved["dataset"]["lock_sha256"]):
        raise RuntimeError("CNS cube validated data lock differs from config")
    checkpoint_lock_path, checkpoint_lock_sha256, checkpoint_lock = (
        create_or_validate_cns_checkpoint_lock(
            root=root,
            core=core,
            config=resolved,
            core_records_path=core_records_path,
    ...
    checkpoints = validate_cns_checkpoint_files(
    ...
    source_files = [
        Path(__file__),
        root / "scripts/analyze_constraint_iclr_pdebench_cns_enforcement_cube.py",
        root / "scripts/analyze_constraint_iclr_pdebench_cns.py",
        root / "scripts/run_constraint_iclr_pdebench_cns.py",
    ...
    if shared_provenance["git_head"] != str(resolved["expected_git_head"]):
        raise RuntimeError("unexpected Git HEAD for CNS enforcement cube")
    if bool(shared_provenance["git_dirty"]) is not bool(
        resolved["expected_git_dirty"]
    ):
        raise RuntimeError("unexpected dirty flag for CNS enforcement cube")
    ...
    for seed in [int(value) for value in resolved["seeds"]]:
        for mechanism, spec in DERIVED_SPEC.items():
            ...
            if sha256_file(checkpoint_path) != checkpoint_entry["sha256"]:
                raise RuntimeError("CNS parent checkpoint changed before evaluation")
            ...
            model = build_cns_model(
                resolved["model"], str(spec["forward_map"])
            ).to(device)
            model.load_state_dict(checkpoint["model_state_dict"], strict=True)
            parameter_count = count_trainable_parameters(model)
            if parameter_count != int(parent["compute"]["trainable_parameters"]):
                raise RuntimeError("CNS cube parent parameter count mismatch")
            if device.type == "cuda":
                torch.cuda.reset_peak_memory_stats(device)
            started = time.perf_counter()
            cases, projection_identity = evaluate_cns_cases(
                model=model,
            ...
            if not all(
                ...
                raise FloatingPointError("non-finite CNS cube metric")
            peak_memory = (
...
@hydra.main(
    ...
    config_name="pdebench_cns_eta0p01_enforcement_cube_20260902",
)
