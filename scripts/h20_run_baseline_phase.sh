#!/usr/bin/env bash
# Run the 095 baseline batch — WGAN-LP + TrajCast-lite fit + sample +
# score, one config = one (asset, model, seed) cell.
#
# Differs from h20_run_phase.sh:
#   - calls scripts/run_baseline_fit_eval.py (not torchrun + train_distributed)
#   - no separate eval pass — fit/sample/score happens in the single script
#   - single-card per config (these models are tiny, no DDP)
...
if [[ -z "$CONFIG_DIR" || ! -d "$CONFIG_DIR" ]]; then
    echo "Usage: $0 <experiments/095_.../>" >&2
    exit 1
fi
...
fit_one_card() {
    ...
    fi
    {
        ...
        local start=$(date +%s)
        local job_log="$outdir/baseline_${TIMESTAMP}.log"
        CUDA_VISIBLE_DEVICES="$card" \
        timeout 3600 conda run -n ecophys python scripts/run_baseline_fit_eval.py \
...
main() {
    echo "═══ BASELINE PHASE — $(date) ═══" | tee -a "$QUEUE_LOG"
