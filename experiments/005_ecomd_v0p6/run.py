"""EcoMD v0.6 — Student-t noise + state-dependent β.

Reuses train_ecomd from ecomd.training.train; outputs six_way_comparison.md
comparing real / GARCH / LM99 / v0 trained / v0.5 trained / v0.6.
"""
...
log = logging.getLogger("ecomd_v0p6")
...
def build_six_way_comparison(v0p6_eval: dict[str, Any], target_dataset: str = "spx") -> str:
    ...
    v0p5_raw = _load_json(V0P5_RESULTS / "ecomd_v0p5_stylized_facts.json") or {}
    v0p5_agg = v0p5_raw.get("aggregated", {})

    v0p6_agg = v0p6_eval["aggregated"]

    lines: list[str] = []
    lines.append(f"# Six-way comparison — real ({target_dataset}) / GARCH / LM99 / EcoMD v0 / v0.5 / v0.6")
    lines.append("")
    lines.append("v0.6 changes over v0.5: (1) Student-t(ν=5) noise replaces Gaussian;")
    lines.append("(2) β is a learnable function of (vol, last_return) instead of constant.")
    lines.append("Training recipe inherited from v0.5 (persistent state + warmup + cosine LR).")
    lines.append("")
    header_cols = ["metric", "expected", f"real ({target_dataset})", "GARCH", "LM99",
                   "EcoMD v0", "EcoMD v0.5", "EcoMD v0.6"]
    ...
    lines.append("")
    lines.append("| metric | GARCH | LM99 | EcoMD v0 | EcoMD v0.5 | EcoMD v0.6 |")
    lines.append("|---|---|---|---|---|---|")
    tot = {"g": 0, "lm": 0, "v0": 0, "v0p5": 0, "v0p6": 0}
    for label, key, _ in ROWS:
        ...
        gc, lc = _score(gv, key), _score(lv, key)
        v0c, v0p5c, v0p6c = _score(v0v, key), _score(v0p5v, key), _score(v0p6v, key)
        tot["g"]    += int(gc)
        tot["lm"]   += int(lc)
        tot["v0"]   += int(v0c)
        tot["v0p5"] += int(v0p5c)
        ...
        lines.append(f"| {label} | {mark(gc)} | {mark(lc)} | {mark(v0c)} | {mark(v0p5c)} | {mark(v0p6c)} |")
    lines.append(f"| **Total** | **{tot['g']}/11** | **{tot['lm']}/11** "
                 f"| **{tot['v0']}/11** | **{tot['v0p5']}/11** | **{tot['v0p6']}/11** |")
    return "\n".join(lines)
...
def main() -> None:
    ...
    log.info(f"EcoMDSimulator built: {sum(p.numel() for p in sim.parameters())} parameters "
             f"(noise_dist={simulator_config.noise_dist}, noise_df={simulator_config.noise_df}, "
             f"learnable_beta={sim.price_formation.params.learnable_beta})")  # type: ignore[attr-defined]
    ...
    (OUT / "ecomd_v0p6_stylized_facts.json").write_text(json.dumps({
        "aggregated": eval_out["aggregated"],
        "realizations": eval_out["realizations"],
        "eval_time_seconds": t_eval,
    }, indent=2))

    md = build_six_way_comparison(eval_out, target_dataset=train_cfg["target_dataset"])
    (OUT / "six_way_comparison.md").write_text(md)
    print(md)
