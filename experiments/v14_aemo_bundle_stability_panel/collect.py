#!/usr/bin/env python3
"""Download the eight frozen panel objects exactly once as opaque bytes."""
...
USER_AGENT = "EcoPhys-AEMO-bundle-stability-panel/1.0"
CHUNK_BYTES = 1 << 20
...
def main() -> None:
    ...
    errors = validate_panel_manifest(manifest, source_sha256=current_manifest_sha)
    if errors:
        raise RuntimeError("invalid AEMO panel manifest: " + "; ".join(errors))
    ...
    objects = cast(list[dict[str, object]], manifest["new_objects"])
    for spec in objects:
        final_path = raw_root / cast(str, spec["expected_filename"])
        partial_path = final_path.with_suffix(final_path.suffix + ".part")
        if final_path.exists() or partial_path.exists():
            raise FileExistsError(f"pre-existing panel payload: {final_path}")
