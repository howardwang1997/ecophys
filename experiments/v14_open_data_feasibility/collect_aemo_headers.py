#!/usr/bin/env python3
"""Download and audit the ten frozen AEMO table archives without joining rows."""
...
def _stream_download(
    ...
    *,
    ...
    for attempt in range(1, TRANSPORT_RETRIES + 2):
        ...
        if response.status_code not in ({206} if existing_bytes else {200}):
            response.close()
        ...
        try:
            ...
            last_error = f"{type(error).__name__}: {error}"
            response.close()
            return None, last_error, attempt, response_headers
        response.close()
        return response.status_code, None, attempt, response_headers
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "data/manifests/aemo_development_objects_v1.yaml",
    )
    parser.add_argument("--resume", action="store_true")
    arguments = parser.parse_args()

    commit = require_clean_repository(ROOT)
    manifest = load_aemo_object_manifest(arguments.manifest)
    errors = validate_aemo_object_manifest(manifest)
    if errors:
        raise RuntimeError("invalid AEMO object manifest: " + "; ".join(errors))
    outputs = cast(dict[str, object], manifest["outputs"])
    raw_root = ROOT / cast(str, outputs["raw_root"])
    ledger_path = ROOT / cast(str, outputs["download_ledger"])
    ...
    if header_path.exists() or summary_path.exists():
        raise FileExistsError("header contract or summary already exists")
    ...
    for index, spec in enumerate(objects[len(downloads) :], start=len(downloads)):
        ...
        if final_path.exists():
            ...
            recovered = False
            successful_status = status == 200 or (status == 206 and partial_path.exists())
                ...
                json.dumps(
                    {
                        ...
                        "bad_crc_member": audit["bad_crc_member"],
                    },
                    sort_keys=True,
                ),
                flush=True,
    ...
    header_contract = build_header_contract(
        audits,
    ...
    summary["ownership"] = {
        "git_commit": commit,
        "object_manifest_sha256": source_hash,
        "generated_at": generated_at,
        "raw_root": raw_root.relative_to(ROOT).as_posix(),
        "raw_archive_bytes": sum(
            cast(int, download["observed_bytes"]) for download in downloads
        ),
