"""EcoMD v0.8 — v0.6 + sigma_price reduction for #10 corr(V,|r|).

Train + eval + produce eight_way_comparison.md (real/GARCH/LM99/v0/v0.5/v0.6/v0.8).
"""
...
log = logging.getLogger("ecomd_v0p8")
...
def build_eight_way_comparison(v0p8_eval: dict[str, Any], target_dataset: str = "spx") -> str:
    ...
    V0P7_RESULTS = REPO / "experiments" / "010_ecomd_v0p7" / "results"
    ...
    v0p7_agg = (_load_json(V0P7_RESULTS / "ecomd_v0p7_stylized_facts.json") or {}).get("aggregated", {})
    v0p8_agg = v0p8_eval["aggregated"]

    lines: list[str] = []
    lines.append(f"# Eight-way comparison — real ({target_dataset}) / GARCH / LM99 / EcoMD v0 / v0.5 / v0.6 / v0.7 / v0.8")
    lines.append("")
    lines.append("v0.8 changes over v0.6: learnable_beta true → false (only change).")
    ...
    for label, key, expected in ROWS:
        row = [label, expected, fmt(real_facts.get(key)), fmt(garch_agg.get(key)),
               ...
               fmt(v0p6_agg.get(key)), fmt(v0p7_agg.get(key)), fmt(v0p8_agg.get(key))]
        lines.append("| " + " | ".join(row) + " |")

    lines.append("")
    lines.append("## Scoreboard")
    lines.append("")
    lines.append("| metric | GARCH | LM99 | v0 | v0.5 | v0.6 | v0.7 | v0.8 |")
    lines.append("|---|---|---|---|---|---|---|---|")
    tot = {"g": 0, "lm": 0, "v0": 0, "v0p5": 0, "v0p6": 0, "v0p7": 0, "v0p8": 0}
    for label, key, _ in ROWS:
        ...
        v0p8v = (v0p8_agg.get(key) or {}).get("mean")
...
def main() -> None:
    ...
    (OUT / "ecomd_v0p8_stylized_facts.json").write_text(json.dumps({
        "aggregated": eval_out["aggregated"],
        "realizations": eval_out["realizations"],
        "eval_time_seconds": t_eval,
    }, indent=2))

    md = build_eight_way_comparison(eval_out, target_dataset=train_cfg["target_dataset"])
    (OUT / "eight_way_comparison.md").write_text(md)
    print(md)
