#!/usr/bin/env python3
"""Collect frozen AEMO ZIP prefixes under the repaired 64 KiB contract."""
...
USER_AGENT = "EcoPhys-AEMO-source-contract-repair/2.0"
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "data/manifests/aemo_source_contract_repair_v2.yaml",
    )
    arguments = parser.parse_args()
    commit = require_clean_repository(ROOT)
    manifest = load_source_contract_manifest(arguments.manifest)
    errors = validate_source_contract_repair_manifest(manifest)
    if errors:
        raise RuntimeError("invalid AEMO source-contract repair: " + "; ".join(errors))
            ...
            json.dumps(
                {
                    ...
                    "exact_range_contract_pass": audit["exact_range_contract_pass"],
                    "zip_prefix_parse": audit["zip_prefix_parse"],
                    "source_header_contract_pass": audit["source_header_contract_pass"],
