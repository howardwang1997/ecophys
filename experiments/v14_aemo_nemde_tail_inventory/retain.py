#!/usr/bin/env python3
"""Retain exact AEMO NEMDE ZIP suffixes in R2 before metadata parsing."""
...
def main() -> None:
    ...
    if download_errors:
        raise RuntimeError("invalid NEMDE-tail download: " + "; ".join(download_errors))
    ...
    if download["source_manifest_sha256"] != manifest_sha256(arguments.manifest):
        raise RuntimeError("download manifest hash differs from current bytes")
    local_errors = verify_local_tails(raw_root, download, manifest)
    if local_errors:
        raise RuntimeError("local NEMDE tails changed: " + "; ".join(local_errors))
    ...
    for source, spec in zip(downloaded, specs, strict=True):
        path = raw_root / cast(str, spec["expected_filename"])
        key = cast(str, spec["r2_key"])
        expected_sha = cast(str, source["sha256"])
        expected_bytes = cast(int, source["observed_bytes"])
        ...
        if head is not None:
            ...
            client.upload_file(
                ...
                bucket,
                key,
                ExtraArgs={"ContentType": "application/octet-stream", "Metadata": {"sha256": expected_sha}},
            )
            uploaded = True
        ...
        remote_sha = cast(dict[str, str], verified_head.get("Metadata", {})).get("sha256")
        ...
        print(json.dumps({"object_id": spec["object_id"], "uploaded": uploaded, "verified": verified}, sort_keys=True), flush=True)
    payload: dict[str, object] = {
        ...
        "objects": retained,
        "all_verified": len(retained) == 2 and all(item["verified"] is True for item in retained),
        "zip_metadata_opened": False,
        "xml_member_opened": False,
    }
    retention_path.parent.mkdir(parents=True, exist_ok=True)
    retention_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
