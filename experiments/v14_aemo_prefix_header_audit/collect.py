#!/usr/bin/env python3
"""Collect frozen AEMO ZIP prefixes and inspect only MMSDM information headers."""
...
USER_AGENT = "EcoPhys-AEMO-prefix-header-audit/1.0"
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "data/manifests/aemo_prefix_header_audit_v4.yaml",
    )
    arguments = parser.parse_args()
    commit = require_clean_repository(ROOT)
    manifest = load_prefix_manifest(arguments.manifest)
    errors = validate_prefix_manifest(manifest)
    if errors:
        raise RuntimeError("invalid prefix-header manifest: " + "; ".join(errors))
