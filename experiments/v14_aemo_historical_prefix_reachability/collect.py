#!/usr/bin/env python3
"""Download two frozen AEMO historical ZIP prefixes as opaque bytes."""
...
USER_AGENT = "EcoPhys-AEMO-historical-prefix-reachability/1.0"
CHUNK_BYTES = 1 << 20


def _download_prefix_once(
            ...
            with partial_path.open("xb") as handle:
                while chunk := response.raw.read(CHUNK_BYTES):
                    ...
                    if observed_bytes > RANGE_BYTES:
                        transport_error = "response exceeded frozen range cap"
                        break
                handle.flush()
...
def main() -> None:
    ...
    for index, spec in enumerate(specs):
        ...
        partial_path = final_path.with_suffix(final_path.suffix + ".part")
        status, observed, transport_error, headers, sha256 = _download_prefix_once(
            session,
            url=cast(str, spec["url"]),
            partial_path=partial_path,
        )
        content_range = headers.get("Content-Range") or headers.get("content-range")
        expected_range = f"bytes 0-{RANGE_BYTES - 1}/{spec['expected_archive_bytes']}"
        passed = (
            status == 206
