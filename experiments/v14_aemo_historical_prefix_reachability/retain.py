#!/usr/bin/env python3
"""Retain exact historical AEMO prefixes in R2 before row parsing."""
...
def main() -> None:
    ...
    if download_errors:
        raise RuntimeError("invalid historical-prefix download: " + "; ".join(download_errors))
    ...
    if local_errors:
        raise RuntimeError("local historical prefixes changed: " + "; ".join(local_errors))
    ...
    for source, spec in zip(downloaded, specs, strict=True):
        ...
        print(
            json.dumps(
                {"object_id": spec["object_id"], "uploaded": uploaded, "verified": verified},
                sort_keys=True,
            ),
            flush=True,
        )
