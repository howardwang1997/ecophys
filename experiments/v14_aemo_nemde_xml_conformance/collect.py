#!/usr/bin/env python3
"""Download two frozen AEMO NEMDE member ranges as opaque bytes."""
...
USER_AGENT = "EcoPhys-AEMO-NEMDE-XML-conformance/1.0"
CHUNK_BYTES = 1 << 20
        ...
        if status == 206:
            ...
            with partial_path.open("xb") as handle:
                while chunk := response.raw.read(CHUNK_BYTES):
                    ...
                    if observed_bytes > RANGE_BYTES:
                        transport_error = "response exceeded frozen member-range cap"
                        break
                handle.flush()
...
def main() -> None:
    ...
    for index, spec in enumerate(specs):
        ...
        partial_path = final_path.with_suffix(final_path.suffix + ".part")
        start = cast(int, spec["range_start"])
        end = cast(int, spec["range_end"])
        status, observed, transport_error, headers, sha256 = _download_range_once(
            session,
        ...
        expected_range = f"bytes {start}-{end}/{spec['expected_archive_bytes']}"
        passed = (
            status == 206
            and transport_error is None
            and observed == RANGE_BYTES
