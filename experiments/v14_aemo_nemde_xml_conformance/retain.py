#!/usr/bin/env python3
"""Retain exact AEMO NEMDE member ranges in R2 before XML parsing."""
...
def main() -> None:
    ...
    if download_errors:
        raise RuntimeError("invalid NEMDE-XML download: " + "; ".join(download_errors))
    ...
    local_errors = verify_local_ranges(raw_root, download, manifest)
    if local_errors:
        raise RuntimeError("local NEMDE ranges changed: " + "; ".join(local_errors))
