#!/usr/bin/env python3
"""Download the exact three opaque AEMO row-conformance objects once."""
...
USER_AGENT = "EcoPhys-AEMO-row-conformance/1.0"
CHUNK_BYTES = 1 << 20
...
def main() -> None:
    ...
    errors = validate_row_conformance_manifest(manifest)
    if errors:
        raise RuntimeError("invalid AEMO row-conformance manifest: " + "; ".join(errors))
    ...
    for spec in objects:
        ...
        if final_path.exists() or partial_path.exists():
            raise FileExistsError(f"pre-existing row-source payload: {final_path}")
