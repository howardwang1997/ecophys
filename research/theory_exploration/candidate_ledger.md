...
The scale attack makes BPO-only transfer insufficient. Prague/Osaka, BPO1 and BPO2 preserve `M/T=1.5`; their
...
## 17. V13 prospective algorithmic-market mechanism changes

| ID | Proposed object | Decisive audit | Current state |
|---|---|---|---|
| V13-H1 | relay removal attenuates builder latency rents | pre-period bids are relay-selected and relay-clocked, Ultra Sound aggregation is not all-relay coverage, and post-ePBS global P2P messages change the observation operator | `RETIRED_IDENTIFIABILITY` |
| V13-H2 | private-information rents persist after relay removal | losing payloads are undisclosed and public-mempool nonappearance does not certify exclusive flow; private valuation is post-selection in winning blocks | `RETIRED_IDENTIFIABILITY` |
| V13-H3 | efficiency can improve without proportional deconcentration | latency, private-flow, efficiency and builder-concentration channels are already modeled directly in MEV-Boost/ePBS work | `RETIRED_PRIOR_ART` |
| V13-NCS | prospectively replicate the transition in two real algorithmic-agent markets | ePBS has no final fork epoch; CoW changes predate the freeze and are not mechanism-isomorphic; Jito BAM is already partially deployed without a demonstrated complete all-action tape | `NO_ADMITTED_NCS_CANDIDATE` |

The exact observation warning is retained. If `E=A or N` is an empty slot generated by strategic withholding `A`
or nonstrategic failure `N`, observations `(E,Z)` alone cannot distinguish the two: any conditional law
`Pr(E=1|Z=z)=p(z)` has both a pure-strategy representation and a pure-failure representation. A predicted
volatility sign is therefore not mechanism identification.

A successor may audit self-concealing mechanism actions only if it adds authenticated multi-vantage propagation,
