"""Outcome-free checks of the exact scientific image, using the existing launcher."""
...
def bind_image_files(root: Path, image: str, work: Path) -> dict[str, str]:
    name = "ecomd-response-sourcecheck-" + uuid.uuid4().hex
    created = False
    hashes: dict[str, str] = {}
    pairs = [(f"/opt/response_dx/{name}", f"research/paper_g/response_dx/{name}")
             for name in ("model.py", "runner.py", "oci_probe.py")]
    pairs.append(("/requirements.lock", "research/paper_g/response_dx/requirements.lock"))
    provenance = json.loads((root / "research/paper_g/response_dx/preparation/dependency_provenance.json").read_text())
    for item in provenance["dependencies"]:
        metadata_ref = next(ref for ref in item["license_files"] if ref.endswith("/METADATA"))
        dist_info = Path(metadata_ref).parent.name
        pairs.append((f"/usr/local/lib/python3.11/site-packages/{dist_info}/METADATA", metadata_ref))
    try:
...
def qualify(root: Path, image: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=".response-dx-conformance-", dir=root) as temporary:
        work = Path(temporary)
        ...
        for name, mode, seconds, limit, expected in (
            ...
            plan = make_plan(work, image, name, mode=mode, cpu_seconds=seconds, output_bytes=limit)
            inspect_local_image(plan)
            status, exit_code, _, _, artifacts = capture_container(plan)
            if status != expected:
                receipt_dir = root / "logs/private/response_dx_conformance" / datetime.now(UTC).strftime("%Y%m%dT%H%M%S")
                ...
                (receipt_dir / "incident.json").write_text(json.dumps({
                    ...
                    "case": name, "status": status, "image": image, "exit_code": exit_code,
                }, indent=2) + "\n")
        ...
        refs = [
            ...
            "research/paper_g/response_dx/model.py",
            "research/paper_g/response_dx/runner.py",
            "research/paper_g/response_dx/Dockerfile",
            "research/paper_g/response_dx/requirements.lock",
            "research/paper_g/response_dx/qualify_image.py",
            "research/discovery/conformance/oci_probe.py",
            "research/paper_g/response_dx/oci_probe.py",
        ]
        return {
            "visibility": "private_internal", "public_evidence_eligible": False,
            "image_digest": image, "completed_at": datetime.now(UTC).isoformat(),
...
def main() -> None:
    ...
    destination = root / "research/paper_g/response_dx/preparation/image_conformance.json"
