...
description: User is an independent full-time researcher pursuing top-tier work on physics-inspired financial market dynamics. Current reachable compute is 2×V100 32 GB plus a separately scheduled A800 40 GB, with H20 excluded from future plans. Data scope may expand behind scientific gates. No formal advisor/lab; Claude serves as critical research partner.
type: user
...
- **Compute**: Current reachable pool is **2×V100 32 GB on two independent nodes plus one A800 40 GB node**. The A800 must remain a separate worker pool and its availability must be probed; it does not retroactively change a frozen two-V100 seed assignment. **H20 is explicitly excluded from future plans.** Express budgets in measured V100-equivalent GPU-hours, benchmark heterogeneous devices separately, and never reduce seeds/horizons/baselines merely to fit available GPUs.
