...
- Exp134 passed a minimal correctly specified aggregate-L5 recovery test on 72 streams/2.16M events and proved
  ...
  controls. G3 is not passed: the messages remain self-generated by a representable logistic family, reset is
  stylized, the baseline is not a continuous-time Hawkes likelihood, and external timestamp/sign and real-data
  holdout validation remain absent. Paid L2 stays gated.
