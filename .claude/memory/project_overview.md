...
- ✓ **EcoMD v2.1 (2026-04-25 night)**: market-microstructure-derived equivariant GNN (permutation-within-type × persistent type-coupling matrix T_θ). After 33 v2.0 configs all stuck at 4/11, two architectural bugs found: (a) Ilinski gauge invariance was misapplied to agent positions (which have absolute meaning), (b) T = identity initialization killed 75% of inter-type pair forces at K=4 types. Fixed via `gauge_enforce=False` + `T_init_mode=ones` + `kyle_enabled=False` + `phi_init_gain=1.0` → **6/11 on Mac**. Subsequent Hawkes×ACF-shape sweep (12 configs, 2026-04-25 late) found `hawkes_kappa=2.0 + w_acf_shape=0.3` synergises: strong Hawkes provides ACF(r²) lag-1 ≈ 0.45 (matches real SPX), shape loss preserves peak-decay geometry → **9/11 on Mac N=200**, top score across all EcoMD variants. Two stragglers (autocorr_returns, conditional_kurtosis) expected to ease at N=10K. H20 production config locked to this recipe.
