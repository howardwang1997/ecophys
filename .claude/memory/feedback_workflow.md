...
description: Mac local development + GitHub code + Cloudflare R2 canonical data + heterogeneous non-H20 compute pools. Current reachable pool is 2×V100 32 GB plus a separately scheduled A800 40 GB; H20 is excluded.
type: feedback
originSessionId: c6748c05-53ac-462d-9535-154e95f91d9f
---
# Workflow

**Rule** (v4 — revised for the heterogeneous pool 2026-09-03):
...
- **Compute workers**:
  - Current reachable pool: two independent V100 32 GB nodes plus one A800 40 GB node. The A800 is a separate hardware pool; never mix it into a frozen V100 shard assignment without a new prospective runtime decision.
...
**Why**: The original 2026-04-23 H20-specific workflow became unavailable. On 2026-08-09 the user explicitly chose a hardware-agnostic plan starting from 2×V100. On 2026-09-03 the current third worker was verified as an A800 40 GB node, replacing the older RTX-worker assumption for forward scheduling. R2/GitHub separation still avoids file-sync ambiguity and makes workers replaceable.
...
- When user says "开始训练" / "start training", probe the V100 and A800 pools, respect every frozen host/seed assignment, and do not infer H20 or idle A800 availability.
